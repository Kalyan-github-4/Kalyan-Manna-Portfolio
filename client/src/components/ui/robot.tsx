      ...
      <div className="w-full max-w-[1400px] mx-auto flex flex-col relative pointer-events-auto">
        <div className="flex flex-col lg:flex-row items-center justify-between relative gap-4 lg:gap-0">
          ...
          <div className="flex flex-wrap justify-center lg:justify-end items-center gap-2 sm:gap-3 w-full lg:w-auto mt-4 lg:mt-0 z-20">
            ...
            >
              {ctaText}
              <ShoppingBag size={16} weight="bold" data-icon="inline-end" />
            </button>
          </div>
        </div>
    ...
    >
      ...
      <div className="absolute inset-0 z-10">
        <Canvas shadows camera={{ position: [0, 0.2, 6], fov: 40 }}>
          ...
          {/* Isolated behind its own boundary on purpose. The preset streams
              an HDR from a CDN, and Canvas wraps all its children in a single
...
/**
 * Just the robot — no navbar, no background wordmark, no gradient. The canvas
 * keeps its default transparent clear colour so whatever the section sits on
 * shows through, which is what lets it drop onto the site's black.
 *
 * RobotHero leaves both directional lights at zero and leans entirely on the
 * studio HDR. That reads as flat grey against a dark page, so this turns them
 * back up: the model still has shape before the environment map arrives, and
 * never depends on a CDN fetch to be visible.
 */
